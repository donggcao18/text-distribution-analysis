...
for i, (label_name, color, marker) in enumerate(zip(valid_embeddings.keys(), colors, markers)):
    ...
    ax.scatter(
        reduced[indices, 0], reduced[indices, 1], reduced[indices, 2],
        label=label_name, c=color, marker=marker, alpha=0.7, s=10
    )
...
plt.title("3D Embedding Space Visualization of AI Text Models", fontsize=14)
plt.tight_layout()
plt.savefig('model_embeddings_visualization.png', dpi=300)
print("Visualization saved as 'model_embeddings_visualization.png'")
plt.show()